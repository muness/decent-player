# (removed) Behavior Analysis — During Active Playback on Cayin RU7 at 44.1kHz
...
- USB device permission: uid 10562 ((removed))

## AudioFlinger State
- **NO AudioFlinger thread has USB as output device** during (removed) playback
...
- Then switched: `Speaker → USB_DEVICE → Speaker` briefly during (removed) init

## (removed) Ghost AudioTrack
```
AudioTrack: uid 10562, Port ID 556, Session 3513
...
Key: (removed) uses `setPreferredDevice(speaker)` on its AudioTrack.
...
```
(removed)'s AudioTrack has `Preferred Device Port ID: 3` = speaker.

## What (removed) Does (Summary)
...
6. USB audio comes EXCLUSIVELY from (removed)'s isochronous transfers

## Clock Source
From our earlier testing: Cayin RU7 clock source entity ID = 0x0B

## Why Our Driver Has No Sound
